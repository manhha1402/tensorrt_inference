...
enum class LogLevel { Trace, Debug, Info, Warn, Error, Critical, Off, Unknown };
...
inline std::string getLogLevelFromEnvironment() {
  ...
  if (envValue) {
    ...
    spdlog::warn("LOG_LEVEL environment variable not set. Using default log "
                 "level (info).");
    return "info";
  }
}
...
inline LogLevel parseLogLevel(const std::string &logLevelStr) {
  if (logLevelStr == "trace") {
    ...
    spdlog::warn("Unknown log level string: {}. Defaulting to 'info' level.",
                 logLevelStr);
