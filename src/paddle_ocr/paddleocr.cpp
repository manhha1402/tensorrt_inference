...
namespace
{
  ...
  static std::vector<std::string> readDict(std::string &path)
  {
...
namespace tensorrt_inference
{
      ...
      : Engine(options)
  {
    ...
    onnx_file_ = (model_dir / model_name / "text_detection.onnx").string();

    if (!std::filesystem::exists(onnx_file_))
    {
      std::cout << onnx_file_ << std::endl;
      throw std::runtime_error("text_detection onnx file not existed");
    }
      ...
      : Engine(options)
  {
    ...
    onnx_file_ = (model_dir / model_name / "text_recognition.onnx").string();
    if (!std::filesystem::exists(onnx_file_))
    {
      std::cout << onnx_file_ << std::endl;
      throw std::runtime_error("text_recognition onnx file not existed");
    }

    std::string labels_file = (model_dir / model_name / "ppocr_keys_v1.txt").string();
    if (!std::filesystem::exists(labels_file))
    {
      throw std::runtime_error("labels file not existed");
    }
