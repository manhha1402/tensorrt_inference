...
std::vector<Object> YoloV9::postprocessDetect(
    std::unordered_map<std::string, std::vector<float>> &feature_vectors,
    const DetectionParams &params,
    const std::vector<std::string> &detected_class) {
  ...
  /*
  cv::Mat res_mat = cv::Mat(CATEGORY + 4, num_anchors, CV_32F,
  feature_vector["output0"].data()); res_mat = res_mat.t(); cv::Mat prob_mat;
...
std::vector<Object> YoloV9::postProcessSegmentation(
    std::unordered_map<std::string, std::vector<float>> &feature_vectors,
    const DetectionParams &params,
    const std::vector<std::string> &detected_class) {
