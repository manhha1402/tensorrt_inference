...
{}
std::vector<Object> YoloV8::postprocess(std::unordered_map<std::string, std::vector<float>> &feature_vectors,const DetectionParams& params)
{
    ...
    if(m_trtEngine->getOutputInfo().size () == 1)
    {
        ...
        {
            //human pose
            return postprocessPose(feature_vectors,params);
        }
        else{
...
std::vector<Object> YoloV8::postprocessDetect(std::unordered_map<std::string, std::vector<float>> &feature_vectors,const DetectionParams& params)
{ 
...
std::vector<Object> YoloV8::postProcessSegmentation(std::unordered_map<std::string, std::vector<float>> &feature_vectors,const DetectionParams& params)
{
...
std::vector<Object> YoloV8::postprocessPose(std::unordered_map<std::string, std::vector<float>> &feature_vectors,const DetectionParams& params)
{
    int num_anchors = m_trtEngine->getOutputInfo().at("output0").dims.d[2];
    int num_channels = m_trtEngine->getOutputInfo().at("output0").dims.d[1];
    std::vector<cv::Rect> bboxes;
