  ...
  for (int i = 0; i < rows; ++i)
    {
        ...
        if (maxClassScore > params.obj_threshold)
        {
            ...
            int left = int((x - 0.5 * w) * factors_[0]);
            int top = int((y - 0.5 * h) * factors_[1]);
            int width = int(w * factors_[0]);
            int height = int(h * factors_[1]);
    ...
    for (auto &chosenIdx : indices) {
      ...
      Object obj{};
      std::cout<<class_labels_[labels[chosenIdx]]<<std::endl;
