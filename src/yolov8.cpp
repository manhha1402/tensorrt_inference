...
void YoloV8::drawBBoxLabel(cv::Mat &image, const Object &object,unsigned int scale)
{
// Choose the color
...
void YoloV8::drawSegmentation(cv::Mat &mask, const Object &object)
{
    // Choose the color
    int colorIndex = object.label % COLOR_LIST.size(); // We have only defined 80 unique colors
    cv::Scalar color = cv::Scalar(COLOR_LIST[colorIndex][0], COLOR_LIST[colorIndex][1], COLOR_LIST[colorIndex][2]);
...
const std::vector<std::string>& detected_class, unsigned int scale) 
{
    ...
    if (!objects.empty() && !objects[0].boxMask.empty()) 
    {
        ...
        for (const auto &object : objects) {
            ...
            {
               if(std::find(detected_class.begin(), detected_class.end(), CLASS_NAMES[object.label])!= detected_class.end())
               {
                 drawSegmentation(mask,object);
               }
            }
    ...
    for (auto &object : objects) {
        if(detected_class.empty())
        {
            drawBBoxLabel(image,object,scale);
        }
        else
        {
            if(std::find(detected_class.begin(), detected_class.end(), CLASS_NAMES[object.label])!= detected_class.end())
